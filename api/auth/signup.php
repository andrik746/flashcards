...
if(isset($postdata) && !empty($postdata)) {
  ...
  // Validate.
  if(trim($request->data->username) === '' && trim($request->data->password) === '' && trim($request->data->email) === '') {
  ...
  $sql_check = "SELECT user_id FROM users WHERE user_login = '" . $username . "'";

  if($result = mysqli_query($con,$sql_check)){

    if(  !empty( mysqli_fetch_assoc($result) )  ){
      echo json_encode(['data'=>'existing user']);
    } else {
    
      $sql_signup = "INSERT INTO users ( user_login, user_password, user_email, `user_registered` ) ";
      $sql_signup .= "SELECT '" . $username . "', PASSWORD('" . $password . "'), '" . $email . "', NOW()";

      if($result = mysqli_query($con,$sql_signup)){
